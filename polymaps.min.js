...
m=v.dblclick(),e=v.arrow();d.map=function(a){i.map(a);c.map(a);m.map(a);e.map(a);return d};return d};v.compass=function(){function d(A){y.setAttribute("class","compass active");N||(N=setInterval(i,o));M&&H.panBy(M);x=Date.now();return f(A)}function i(){M&&Date.now()>x+j&&H.panBy(M)}function c(A){if(A.shiftKey){J={x0:H.mouse(A)};H.focusableParent().focus();return f(A)}}function m(A){if(J){J.x1=H.mouse(A);Q.setAttribute("x",Math.min(J.x0.x,J.x1.x));Q.setAttribute("y",Math.min(J.x0.y,J.x1.y));Q.setAttribute("width",
Math.abs(J.x0.x-J.x1.x));Q.setAttribute("height",Math.abs(J.x0.y-J.x1.y));Q.removeAttribute("display")}}function e(){y.setAttribute("class","compass");if(J){if(J.x1){H.extent([H.pointLocation({x:Math.min(J.x0.x,J.x1.x),y:Math.max(J.x0.y,J.x1.y)}),H.pointLocation({x:Math.max(J.x0.x,J.x1.x),y:Math.min(J.x0.y,J.x1.y)})]);Q.setAttribute("display","none")}J=null}if(N){clearInterval(N);N=0}}function a(A){return function(){A?this.setAttribute("class","active"):this.removeAttribute("class");M=A}}function h(A){return function(G){y.setAttribute("class",
"compass active");var I=H.zoom();H.zoom(A<0?Math.ceil(I)-1:Math.floor(I)+1);return f(G)}}function k(A){return function(G){H.zoom(A);return f(G)}}function n(){this.setAttribute("class","active")}function q(){this.removeAttribute("class")}function f(A){A.stopPropagation();A.preventDefault();return false}function g(A){var G=Math.SQRT1_2*l,I=l*0.7,K=l*0.2,L=v.svg("g"),P=L.appendChild(v.svg("path")),W=L.appendChild(v.svg("path"));P.setAttribute("class","direction");P.setAttribute("pointer-events","all");
P.setAttribute("d","M0,0L"+G+","+G+"A"+l+","+l+" 0 0,1 "+-G+","+G+"Z");W.setAttribute("class","chevron");W.setAttribute("d","M"+K+","+(I-K)+"L0,"+I+" "+-K+","+(I-K));W.setAttribute("pointer-events","none");L.addEventListener("mousedown",d,false);L.addEventListener("mouseover",a(A),false);L.addEventListener("mouseout",a(null),false);L.addEventListener("dblclick",f,false);return L}function u(A){var G=l*0.4,I=G/2,K=v.svg("g"),L=K.appendChild(v.svg("path")),P=K.appendChild(v.svg("path")),W=K.appendChild(v.svg("path")),
X=K.appendChild(v.svg("path"));L.setAttribute("class","back");L.setAttribute("d","M"+-G+",0V"+-G+"A"+G+","+G+" 0 1,1 "+G+","+-G+"V0Z");P.setAttribute("class","direction");P.setAttribute("d",L.getAttribute("d"));W.setAttribute("class","chevron");W.setAttribute("d","M"+-I+","+-G+"H"+I+(A>0?"M0,"+(-G-I)+"V"+-I:""));X.setAttribute("class","fore");X.setAttribute("fill","none");X.setAttribute("d",L.getAttribute("d"));K.addEventListener("mousedown",h(A),false);K.addEventListener("mouseover",n,false);K.addEventListener("mouseout",
q,false);K.addEventListener("dblclick",f,false);return K}function s(A){var G=l*0.2,I=l*0.4,K=v.svg("g"),L=K.appendChild(v.svg("rect")),P=K.appendChild(v.svg("path"));L.setAttribute("pointer-events","all");L.setAttribute("fill","none");L.setAttribute("x",-I);L.setAttribute("y",-0.75*I);L.setAttribute("width",2*I);L.setAttribute("height",1.5*I);P.setAttribute("class","chevron");P.setAttribute("d","M"+-G+",0H"+G);K.addEventListener("mousedown",k(A),false);K.addEventListener("dblclick",f,false);return K}
function w(){var A=l+6,G=A,I=H.size();switch(t){case "top-left":break;case "top-right":A=I.x-A;break;case "bottom-left":G=I.y-G;break;case "bottom-right":A=I.x-A;G=I.y-G;break}y.setAttribute("transform","translate("+A+","+G+")");Q.setAttribute("transform","translate("+-A+","+-G+")");for(var K in F)K==H.zoom()?F[K].setAttribute("class","active"):F[K].removeAttribute("class")}function z(){for(;y.lastChild;)y.removeChild(y.lastChild);y.appendChild(Q);if(B!="none"){T=y.appendChild(v.svg("g"));T.setAttribute("class",
"pan");var A=T.appendChild(v.svg("circle"));A.setAttribute("class","back");A.setAttribute("r",l);T.appendChild(g({x:0,y:-r})).setAttribute("transform","rotate(0)");T.appendChild(g({x:r,y:0})).setAttribute("transform","rotate(90)");T.appendChild(g({x:0,y:r})).setAttribute("transform","rotate(180)");T.appendChild(g({x:-r,y:0})).setAttribute("transform","rotate(270)");A=T.appendChild(v.svg("circle"));A.setAttribute("fill","none");A.setAttribute("class","fore");A.setAttribute("r",l)}else T=null;if(C!=
"none"){D=y.appendChild(v.svg("g"));D.setAttribute("class","zoom");A=-0.5;if(C=="big"){F={};var G=H.zoomRange()[0];for(A=0;G<=H.zoomRange()[1];G++,A++)(F[G]=D.appendChild(s(G))).setAttribute("transform","translate(0,"+-(A+0.75)*l*0.4+")")}G=B=="none"?0.4:2;D.setAttribute("transform","translate(0,"+l*(/^top-/.test(t)?G+(A+0.5)*0.4:-G)+")");D.appendChild(u(+1)).setAttribute("transform","translate(0,"+-(A+0.5)*l*0.4+")");D.appendChild(u(-1)).setAttribute("transform","scale(-1)")}else D=null;w()}var E=
{},y=v.svg("g"),F={},l=30,r=16,x=0,j=250,o=50,t="top-left",C="small",D,B="small",N,M,T,J,Q=v.svg("rect"),H,R,O;y.setAttribute("class","compass");Q.setAttribute("class","back fore");Q.setAttribute("pointer-events","none");Q.setAttribute("display","none");E.radius=function(A){if(!arguments.length)return l;l=A;H&&z();return E};E.speed=function(A){if(!arguments.length)return l;r=A;return E};E.position=function(A){if(!arguments.length)return t;t=A;H&&z();return E};E.pan=function(A){if(!arguments.length)return B;
B=A;H&&z();return E};E.zoom=function(A){if(!arguments.length)return C;C=A;H&&z();return E};E.map=function(A){if(!arguments.length)return H;if(H){R.removeEventListener("mousedown",c,false);R.removeChild(y);R=null;O.removeEventListener("mousemove",m,false);O.removeEventListener("mouseup",e,false);O=null;H.off("move",w).off("resize",w)}if(H=A){R=H.container();R.appendChild(y);R.addEventListener("mousedown",c,false);O=R.ownerDocument.defaultView;O.addEventListener("mousemove",m,false);O.addEventListener("mouseup",
