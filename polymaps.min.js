...
"application/json")},image:function(f,j,s){function r(){k=document.createElement("img");k.onerror=function(){e--;d()};k.onload=function(){e--;s(k);d()};k.src=j;f.setAttributeNS(t.ns.xlink,"href",j)}var k;m.push(r);d();return{abort:function(i){if(g(r))return true;if(i&&k){k.src="about:";return true}return false}}}}}();t.map=function(){function d(){if(v)if(r<v[0])r=v[0];else if(r>v[1])r=v[1];k=r-(r=Math.round(r));i=Math.pow(2,k)}function g(){if(o){var h=45/Math.pow(2,r+k-3),n=Math.max(Math.abs(y*f.x+
w*f.y),Math.abs(z*f.x+E*f.y)),q=Y(G-n*h/j.y);n=Y(l+n*h/j.y);s.lat=Math.max(q,Math.min(n,s.lat));q=Math.max(Math.abs(y*f.y+w*f.x),Math.abs(z*f.y+E*f.x));s.lon=Math.max(o[0].lon-q*h/j.x,Math.min(o[1].lon+q*h/j.x,s.lon))}}var c={},m,e,a=$,f=$,j={x:256,y:256},s={lat:37.76487,lon:-122.41948},r=12,k=0,i=1,v=[1,18],u=0,w=1,y=0,E=1,z=0,G=-180,l=180,o=[{lat:Y(G),lon:-Infinity},{lat:Y(l),lon:Infinity}];c.locationCoordinate=function(h){h=t.map.locationCoordinate(h);var n=Math.pow(2,r);h.column*=n;h.row*=n;h.zoom+=
r;return h};c.coordinateLocation=t.map.coordinateLocation;c.coordinatePoint=function(h,n){var q=Math.pow(2,r-n.zoom),C=Math.pow(2,r-h.zoom),D=(n.column*q-h.column*C)*j.x*i;q=(n.row*q-h.row*C)*j.y*i;return{x:f.x+w*D-y*q,y:f.y+y*D+w*q}};c.pointCoordinate=function(h,n){var q=Math.pow(2,r-h.zoom),C=(n.x-f.x)/i,D=(n.y-f.y)/i;return{column:h.column*q+(E*C-z*D)/j.x,row:h.row*q+(z*C+E*D)/j.y,zoom:r}};c.locationPoint=function(h){var n=Math.pow(2,r+k-3)/45,q=(h.lon-s.lon)*n*j.x;h=(Z(s.lat)-Z(h.lat))*n*j.y;
return{x:f.x+w*q-y*h,y:f.y+y*q+w*h}};c.pointLocation=function(h){var n=45/Math.pow(2,r+k-3),q=(h.x-f.x)*n;h=(h.y-f.y)*n;return{lon:s.lon+(E*q-z*h)/j.x,lat:Y(Z(s.lat)-(z*q+E*h)/j.y)}};var x=t.svg("rect");x.setAttribute("visibility","hidden");x.setAttribute("pointer-events","all");c.container=function(h){if(!arguments.length)return m;m=h;m.setAttribute("class","map");m.appendChild(x);return c.resize()};c.focusableParent=function(){for(var h=m;h;h=h.parentNode)if(h.tabIndex>=0)return h;return window};
c.mouse=function(h){var n=(m.ownerSVGElement||m).createSVGPoint();if(ba<0&&(window.scrollX||window.scrollY)){var q=document.body.appendChild(t.svg("svg"));q.style.position="absolute";q.style.top=q.style.left="0px";var C=q.getScreenCTM();ba=!(C.f||C.e);document.body.removeChild(q)}if(ba){n.x=h.pageX;n.y=h.pageY}else{n.x=h.clientX;n.y=h.clientY}return n.matrixTransform(m.getScreenCTM().inverse())};c.size=function(h){if(!arguments.length)return a;e=h;return c.resize()};c.resize=function(){if(e){a=e;
S.remove(c)}else{x.setAttribute("width","100%");x.setAttribute("height","100%");b=x.getBBox();a={x:b.width,y:b.height};S.add(c)}x.setAttribute("width",a.x);x.setAttribute("height",a.y);f={x:a.x/2,y:a.y/2};g();c.dispatch({type:"resize"});return c};c.tileSize=function(h){if(!arguments.length)return j;j=h;c.dispatch({type:"move"});return c};c.center=function(h){if(!arguments.length)return s;s=h;g();c.dispatch({type:"move"});return c};c.panBy=function(h){var n=45/Math.pow(2,r+k-3),q=h.x*n;h=h.y*n;return c.center({lon:s.lon+
(z*h-E*q)/j.x,lat:Y(Z(s.lat)+(z*q+E*h)/j.y)})};c.centerRange=function(h){if(!arguments.length)return o;if(o=h){G=o[0].lat>-90?Z(o[0].lat):-Infinity;l=o[0].lat<90?Z(o[1].lat):Infinity}else{G=-Infinity;l=Infinity}g();c.dispatch({type:"move"});return c};c.zoom=function(h){if(!arguments.length)return r+k;r=h;d();return c.center(s)};c.zoomBy=function(h,n,q){if(arguments.length<2)return c.zoom(r+k+h);if(arguments.length<3)q=c.pointLocation(n);r=r+k+h;d();var C=c.locationPoint(q);return c.panBy({x:n.x-C.x,
y:n.y-C.y})};c.zoomRange=function(h){if(!arguments.length)return v;v=h;return c.zoom(r+k)};c.extent=function(h){if(!arguments.length)return[c.pointLocation({x:0,y:a.y}),c.pointLocation({x:a.x,y:0})];var n=c.locationPoint(h[0]),q=c.locationPoint(h[1]),C=Math.max((q.x-n.x)/a.x,(n.y-q.y)/a.y);n=c.pointLocation({x:(n.x+q.x)/2,y:(n.y+q.y)/2});r=r+k-Math.log(C)/Math.LN2;d();return c.center(n)};c.angle=function(h){if(!arguments.length)return u;u=h;w=Math.cos(u);y=Math.sin(u);E=Math.cos(-u);z=Math.sin(-u);
...
1/360;return{column:(d.lon+180)*g,row:(180-Z(d.lat))*g,zoom:0}};t.map.coordinateLocation=function(d){var g=45/Math.pow(2,d.zoom-3);return{lon:g*d.column-180,lat:Y(180-g*d.row)}};var ba=/WebKit/.test(navigator.userAgent)?-1:0;t.layer=function(d,g){function c(z){for(var G=y[0].nextSibling;w<z;w++){v.insertBefore(y[-4],G);v.insertBefore(y[2],G);v.insertBefore(y[1],G);for(var l=y[-4],o=-4;o<2;)y[o]=y[++o];y[o]=l}}function m(z){for(var G=y[0].nextSibling;w>z;w--){v.insertBefore(y[-1],G);v.insertBefore(y[2],
y[-4]);for(var l=y[2],o=2;o>-4;)y[o]=y[--o];y[o]=l}}function e(){function z(O){var A=O.zoom,F=A<0?1:1<<A,I=O.column%F,K=O.row;if(I<0)I+=F;return{locationPoint:function(L){L=t.map.locationCoordinate(L);var P=Math.pow(2,A-L.zoom);return{x:C.x*(P*L.column-I),y:C.y*(P*L.row-K)}}}}function G(O,A,F){var I=M.zoom,K=2-Q,L=4+Q;for(O=O;O<A;O++){var P=f.load({column:O,row:F,zoom:I},z);if(!P.ready&&!(P.key in H)){P.proxyRefs={};for(var W,X,U,V=1;V<=K;V++){X=true;for(var ca=0,ga=1<<V;ca<=ga;ca++)for(var da=0;da<=
ga;da++)if((U=f.peek(W={column:(O<<V)+da,row:(F<<V)+ca,zoom:I+V}))&&U.ready){H[U.key]=f.load(W);U.proxyCount++;P.proxyRefs[U.key]=U}else X=false;if(X)break}if(!X)for(V=1;V<=L;V++)if((U=f.peek(W={column:O>>V,row:F>>V,zoom:I-V}))&&U.ready){H[U.key]=f.load(W);U.proxyCount++;P.proxyRefs[U.key]=U;break}}H[P.key]=P}}function l(O){return Math.round(O*256)/256}var o=a.map(),x=o.zoom(),h=x-(x=Math.round(x)),n=o.size(),q=o.angle(),C=o.tileSize(),D=o.locationCoordinate(o.center());if(w!=x){if(w<x)c(x);else if(w>
x)m(x);else w=x;for(var B=-4;B<=2;B++){var N=y[B];N.setAttribute("class","zoom"+(B<0?"":"+")+B+" zoom"+(x+B));N.setAttribute("transform","scale("+Math.pow(2,-B)+")")}}var M=o.pointCoordinate(D,$);B=o.pointCoordinate(D,{x:n.x,y:0});x=o.pointCoordinate(D,n);o=o.pointCoordinate(D,{x:0,y:n.y});N=D.column;var T=D.row;D.column=Math.round((Math.round(C.x*D.column)+(n.x&1)/2)/C.x);D.row=Math.round((Math.round(C.y*D.row)+(n.y&1)/2)/C.y);N-=D.column;T-=D.row;var J=l(Math.pow(2,h));v.setAttribute("transform",
"translate("+Math.round(n.x/2-N*C.x*J)+","+Math.round(n.y/2-T*C.y*J)+")"+(q?"rotate("+q/Math.PI*180+")":"")+(h?"scale("+J+")":"")+(u?u.zoomFraction(h):""));if(u){M=u.unapply(M);B=u.unapply(B);x=u.unapply(x);o=u.unapply(o);D=u.unapply(D)}var Q=r?r(M.zoom)-M.zoom:0;if(Q){n=Math.pow(2,Q);M.column*=n;M.row*=n;B.column*=n;B.row*=n;x.column*=n;x.row*=n;o.column*=n;o.row*=n;M.zoom=B.zoom=x.zoom=o.zoom+=Q}h=f.locks();var H={};for(var R in h)h[R].proxyCount=0;if(s&&Q>-5&&Q<3){q=M.zoom<0?1:1<<M.zoom;if(j){fa(M,
B,x,0,q,G);fa(x,o,M,0,q,G)}else{n=Math.floor((M.column+x.column)/2);q=Math.max(0,Math.min(q-1,Math.floor((B.row+o.row)/2)));B=Math.min(4,M.zoom);n=n>>B<<B;q=q>>B<<B;G(n,n+1,q)}}for(R in H){B=H[R];n=l(Math.pow(2,B.level=B.zoom-D.zoom));B.element.setAttribute("transform","translate("+Math.round(B.x=C.x*(B.column-D.column*n))+","+Math.round(B.y=C.y*(B.row-D.row*n))+")")}for(R in h)if(!(R in H)){B=f.unload(R);B.element.parentNode.removeChild(B.element);delete B.proxyRefs}for(R in H){B=H[R];if(B.element.parentNode!=
...
c.request.abort(true)}),g;d.url=function(c){if(!arguments.length)return g;g=typeof c=="string"&&/{.}/.test(c)?t.url(c):c;return d.reload()};return d};t.geoJson=function(d){function g(l){var o={lat:0,lon:0};return function(x){o.lat=x[1];o.lon=x[0];var h=l(o);x.x=h.x;x.y=h.y;return h}}function c(l,o){return l&&l.type in y&&y[l.type](l,o)}function m(l,o,x){return l.type in E&&E[l.type](l,o,x)}function e(){var l=a.map().zoom(),o=a.cache.locks(),x,h,n,q,C,D,B;if(u=="fixed")for(x in o){if((h=o[x]).scale!=
l){B="scale("+Math.pow(2,h.zoom-l)+")";q=-1;for(C=(n=h.features).length;++q<C;)m((D=n[q]).data.geometry,D.element,B);h.scale=l}}else for(x in o){q=-1;for(C=(n=(h=o[x]).features).length;++q<C;)m((D=n[q]).data.geometry,D.element,"");delete h.scale}}var a=t.layer(function(l,o){function x(n){var q=[];if(n.next)l.request=d(n.next.href,x);switch(n.type){case "FeatureCollection":for(var C=0;C<n.features.length;C++){var D=n.features[C],B=c(D.geometry,o);B&&q.push({element:h.appendChild(B),data:D})}break;
case "Feature":(B=c(n.geometry,o))&&q.push({element:h.appendChild(B),data:n});break;default:(B=c(n,o))&&q.push({element:h.appendChild(B),data:{type:"Feature",geometry:n}});break}l.ready=true;q.push.apply(l.features,q);a.dispatch({type:"load",tile:l,features:q})}var h=l.element=t.svg("g");l.features=[];o=g(o(l).locationPoint);if(j!=null)l.request=d(typeof j=="function"?j(l):j,x);else x({type:"FeatureCollection",features:w||[]})},function(l){l.request&&l.request.abort(true)}),f=a.container(),j,s=true,
r="org.polymaps."+t.id(),k="url(#"+r+")",i=f.insertBefore(t.svg("clipPath"),f.firstChild),v=i.appendChild(t.svg("rect")),u="auto",w;f.setAttribute("fill-rule","evenodd");i.setAttribute("id",r);if(!arguments.length)d=t.queue.json;var y={Point:function(l,o){var x=o(l.coordinates),h=t.svg("circle");h.setAttribute("r",4.5);h.setAttribute("transform","translate("+x.x+","+x.y+")");return h},MultiPoint:function(l,o){for(var x=t.svg("g"),h=l.coordinates,n,q,C=-1,D=h.length;++C<D;){q=x.appendChild(t.svg("circle"));
q.setAttribute("r",4.5);q.setAttribute("transform","translate("+(n=o(h[C])).x+","+n.y+")")}return x},LineString:function(l,o){for(var x=t.svg("path"),h=["M"],n=l.coordinates,q,C=-1,D=n.length;++C<D;)h.push((q=o(n[C])).x,",",q.y,"L");h.pop();if(h.length){x.setAttribute("d",h.join(""));return x}},MultiLineString:function(l,o){for(var x=t.svg("path"),h=[],n=l.coordinates,q,C=-1,D,B=n.length,N;++C<B;){q=n[C];D=-1;N=q.length;for(h.push("M");++D<N;)h.push((p=o(q[D])).x,",",p.y,"L");h.pop()}if(h.length){x.setAttribute("d",
h.join(""));return x}},Polygon:function(l,o){for(var x=t.svg("path"),h=[],n=l.coordinates,q,C=-1,D,B=n.length,N;++C<B;){q=n[C];D=-1;N=q.length-1;for(h.push("M");++D<N;)h.push((p=o(q[D])).x,",",p.y,"L");h[h.length-1]="Z"}if(h.length){x.setAttribute("d",h.join(""));return x}},MultiPolygon:function(l,o){for(var x=t.svg("path"),h=[],n=l.coordinates,q,C,D=-1,B,N,M=n.length,T,J;++D<M;){q=n[D];B=-1;for(T=q.length;++B<T;){C=q[B];N=-1;J=C.length-1;for(h.push("M");++N<J;)h.push((p=o(C[N])).x,",",p.y,"L");h[h.length-
1]="Z"}}if(h.length){x.setAttribute("d",h.join(""));return x}},GeometryCollection:function(l,o){for(var x=t.svg("g"),h=-1,n=l.geometries,q=n.length,C;++h<q;)(C=c(n[h],o))&&x.appendChild(C);return x}},E={Point:function(l,o,x){l=l.coordinates;o.setAttribute("transform","translate("+l.x+","+l.y+")"+x)},MultiPoint:function(l,o,x){l=l.coordinates;var h=-1,n=q.length;o=o.firstChild;for(var q;++h<n;){q=l[h];o.setAttribute("transform","translate("+q.x+","+q.y+")"+x);o=o.nextSibling}}};a.url=function(l){if(!arguments.length)return j;
j=typeof l=="string"&&/{.}/.test(l)?t.url(l):l;if(j!=null)w=null;typeof j=="string"&&a.tile(false);return a.reload()};a.features=function(l){if(!arguments.length)return w;if(w=l){j=null;a.tile(false)}return a.reload()};a.clip=function(l){if(!arguments.length)return s;s&&f.removeChild(i);if(s=l)f.insertBefore(i,f.firstChild);var o=a.cache.locks();for(var x in o)s?o[x].element.setAttribute("clip-path",k):o[x].element.removeAttribute("clip-path");return a};var z=a.tile;a.tile=function(l){arguments.length&&
!l&&a.clip(l);return z.apply(a,arguments)};var G=a.map;a.map=function(l){if(l&&v){var o=l.tileSize();v.setAttribute("width",o.x);v.setAttribute("height",o.y)}return G.apply(a,arguments)};a.scale=function(l){if(!arguments.length)return u;(u=l)?a.on("move",e):a.off("move",e);a.map()&&e();return a};a.show=function(l){s?l.element.setAttribute("clip-path",k):l.element.removeAttribute("clip-path");a.dispatch({type:"show",tile:l,features:l.features});return a};a.reshow=function(){var l=a.cache.locks();for(var o in l)a.show(l[o]);
...
w[1].x-w[0].x;l=w[1].y-w[0].y;z.angle=Math.atan2(l,G)}y=z}else y={};y=y;y.n=w.length;if(y&&(r||u!=="current"))y.location=a.pointLocation(y);return y}function m(u){function w(x){d(x.changedTouches,"current");x.preventDefault()}function y(x){d(x.changedTouches,"finish");document.removeEventListener("touchmove",w,false);document.removeEventListener("touchend",y,false);document.removeEventListener("touchcancel",y,false);delete o.__polymaps_touch_listeners__}d(u.changedTouches,"start");for(var E={},z=
-1,G=u.touches.length;++z<G;)E[u.touches[z].identifier]=true;for(var l in k)E[l]||delete k[l];var o=u.target;if(!o.__polymaps_touch_listeners__){document.addEventListener("touchmove",w,false);document.addEventListener("touchend",y,false);document.addEventListener("touchcancel",y,false);o.__polymaps_touch_listeners__=true}}var e={},a,f,j,s,r,k={},i={},v=null;e.rotate=function(u){if(!arguments.length)return r;r=u;return e};e.map=function(u){if(!arguments.length)return a;if(a){f.removeEventListener("touchstart",
m,false);f=null}if(a=u){f=a.container();f.addEventListener("touchstart",m,false)}return e};return e};t.interact=function(){var d={},g=t.drag(),c=t.wheel(),m=t.dblclick(),e=t.touch(),a=t.arrow();d.map=function(f){g.map(f);c.map(f);m.map(f);e.map(f);a.map(f);return d};return d};t.compass=function(){function d(A){z.setAttribute("class","compass active");N||(N=setInterval(g,n));M&&H.panBy(M);x=Date.now();return k(A)}function g(){M&&Date.now()>x+h&&H.panBy(M)}function c(A){if(A.shiftKey){J={x0:H.mouse(A)};
H.focusableParent().focus();return k(A)}}function m(A){if(J){J.x1=H.mouse(A);Q.setAttribute("x",Math.min(J.x0.x,J.x1.x));Q.setAttribute("y",Math.min(J.x0.y,J.x1.y));Q.setAttribute("width",Math.abs(J.x0.x-J.x1.x));Q.setAttribute("height",Math.abs(J.x0.y-J.x1.y));Q.removeAttribute("display")}}function e(){z.setAttribute("class","compass");if(J){if(J.x1){H.extent([H.pointLocation({x:Math.min(J.x0.x,J.x1.x),y:Math.max(J.x0.y,J.x1.y)}),H.pointLocation({x:Math.max(J.x0.x,J.x1.x),y:Math.min(J.x0.y,J.x1.y)})]);
Q.setAttribute("display","none")}J=null}if(N){clearInterval(N);N=0}}function a(A){return function(){A?this.setAttribute("class","active"):this.removeAttribute("class");M=A}}function f(A){return function(F){z.setAttribute("class","compass active");var I=H.zoom();H.zoom(A<0?Math.ceil(I)-1:Math.floor(I)+1);return k(F)}}function j(A){return function(F){H.zoom(A);return k(F)}}function s(){this.setAttribute("class","active")}function r(){this.removeAttribute("class")}function k(A){A.stopPropagation();A.preventDefault();
return false}function i(A){var F=Math.SQRT1_2*l,I=l*0.7,K=l*0.2,L=t.svg("g"),P=L.appendChild(t.svg("path")),W=L.appendChild(t.svg("path"));P.setAttribute("class","direction");P.setAttribute("pointer-events","all");P.setAttribute("d","M0,0L"+F+","+F+"A"+l+","+l+" 0 0,1 "+-F+","+F+"Z");W.setAttribute("class","chevron");W.setAttribute("d","M"+K+","+(I-K)+"L0,"+I+" "+-K+","+(I-K));W.setAttribute("pointer-events","none");L.addEventListener("mousedown",d,false);L.addEventListener("mouseover",a(A),false);
L.addEventListener("mouseout",a(null),false);L.addEventListener("dblclick",k,false);return L}function v(A){var F=l*0.4,I=F/2,K=t.svg("g"),L=K.appendChild(t.svg("path")),P=K.appendChild(t.svg("path")),W=K.appendChild(t.svg("path")),X=K.appendChild(t.svg("path"));L.setAttribute("class","back");L.setAttribute("d","M"+-F+",0V"+-F+"A"+F+","+F+" 0 1,1 "+F+","+-F+"V0Z");P.setAttribute("class","direction");P.setAttribute("d",L.getAttribute("d"));W.setAttribute("class","chevron");W.setAttribute("d","M"+-I+
","+-F+"H"+I+(A>0?"M0,"+(-F-I)+"V"+-I:""));X.setAttribute("class","fore");X.setAttribute("fill","none");X.setAttribute("d",L.getAttribute("d"));K.addEventListener("mousedown",f(A),false);K.addEventListener("mouseover",s,false);K.addEventListener("mouseout",r,false);K.addEventListener("dblclick",k,false);return K}function u(A){var F=l*0.2,I=l*0.4,K=t.svg("g"),L=K.appendChild(t.svg("rect")),P=K.appendChild(t.svg("path"));L.setAttribute("pointer-events","all");L.setAttribute("fill","none");L.setAttribute("x",
-I);L.setAttribute("y",-0.75*I);L.setAttribute("width",2*I);L.setAttribute("height",1.5*I);P.setAttribute("class","chevron");P.setAttribute("d","M"+-F+",0H"+F);K.addEventListener("mousedown",j(A),false);K.addEventListener("dblclick",k,false);return K}function w(){var A=l+6,F=A,I=H.size();switch(q){case "top-left":break;case "top-right":A=I.x-A;break;case "bottom-left":F=I.y-F;break;case "bottom-right":A=I.x-A;F=I.y-F;break}z.setAttribute("transform","translate("+A+","+F+")");Q.setAttribute("transform",
"translate("+-A+","+-F+")");for(var K in G)K==H.zoom()?G[K].setAttribute("class","active"):G[K].removeAttribute("class")}function y(){for(;z.lastChild;)z.removeChild(z.lastChild);z.appendChild(Q);if(B!="none"){T=z.appendChild(t.svg("g"));T.setAttribute("class","pan");var A=T.appendChild(t.svg("circle"));A.setAttribute("class","back");A.setAttribute("r",l);T.appendChild(i({x:0,y:-o})).setAttribute("transform","rotate(0)");T.appendChild(i({x:o,y:0})).setAttribute("transform","rotate(90)");T.appendChild(i({x:0,
y:o})).setAttribute("transform","rotate(180)");T.appendChild(i({x:-o,y:0})).setAttribute("transform","rotate(270)");A=T.appendChild(t.svg("circle"));A.setAttribute("fill","none");A.setAttribute("class","fore");A.setAttribute("r",l)}else T=null;if(C!="none"){D=z.appendChild(t.svg("g"));D.setAttribute("class","zoom");A=-0.5;if(C=="big"){G={};var F=H.zoomRange()[0];for(A=0;F<=H.zoomRange()[1];F++,A++)(G[F]=D.appendChild(u(F))).setAttribute("transform","translate(0,"+-(A+0.75)*l*0.4+")")}F=B=="none"?
0.4:2;D.setAttribute("transform","translate(0,"+l*(/^top-/.test(q)?F+(A+0.5)*0.4:-F)+")");D.appendChild(v(+1)).setAttribute("transform","translate(0,"+-(A+0.5)*l*0.4+")");D.appendChild(v(-1)).setAttribute("transform","scale(-1)")}else D=null;w()}var E={},z=t.svg("g"),G={},l=30,o=16,x=0,h=250,n=50,q="top-left",C="small",D,B="small",N,M,T,J,Q=t.svg("rect"),H,R,O;z.setAttribute("class","compass");Q.setAttribute("class","back fore");Q.setAttribute("pointer-events","none");Q.setAttribute("display","none");
E.radius=function(A){if(!arguments.length)return l;l=A;H&&y();return E};E.speed=function(A){if(!arguments.length)return l;o=A;return E};E.position=function(A){if(!arguments.length)return q;q=A;H&&y();return E};E.pan=function(A){if(!arguments.length)return B;B=A;H&&y();return E};E.zoom=function(A){if(!arguments.length)return C;C=A;H&&y();return E};E.map=function(A){if(!arguments.length)return H;if(H){R.removeEventListener("mousedown",c,false);R.removeChild(z);R=null;O.removeEventListener("mousemove",
m,false);O.removeEventListener("mouseup",e,false);O=null;H.off("move",w).off("resize",w)}if(H=A){R=H.container();R.appendChild(z);R.addEventListener("mousedown",c,false);O=R.ownerDocument.defaultView;O.addEventListener("mousemove",m,false);O.addEventListener("mouseup",e,false);H.on("move",w).on("resize",w);y()}return E};return E};t.grid=function(){function d(){var e=m.firstChild,a=c.size(),f=c.pointLocation($);c.pointLocation(a);var j=Math.pow(2,4-Math.round(c.zoom()));f.lat=Math.floor(f.lat/j)*j;
f.lon=Math.ceil(f.lon/j)*j;for(var s;(s=c.locationPoint(f).x)<=a.x;f.lon+=j){e||(e=m.appendChild(t.svg("line")));e.setAttribute("x1",s);e.setAttribute("x2",s);e.setAttribute("y1",0);e.setAttribute("y2",a.y);e=e.nextSibling}for(;(s=c.locationPoint(f).y)<=a.y;f.lat-=j){e||(e=m.appendChild(t.svg("line")));e.setAttribute("y1",s);e.setAttribute("y2",s);e.setAttribute("x1",0);e.setAttribute("x2",a.x);e=e.nextSibling}for(;e;){a=e.nextSibling;m.removeChild(e);e=a}}var g={},c,m=t.svg("g");m.setAttribute("class",
"grid");g.map=function(e){if(!arguments.length)return c;if(c){m.parentNode.removeChild(m);c.off("move",d).off("resize",d)}if(c=e){c.on("move",d).on("resize",d);c.container().appendChild(m);c.dispatch({type:"move"})}return g};return g};t.stylist=function(){function d(e){var a=e.features.length,f=g.length,j=c.length,s,r,k,i,v,u;for(v=0;v<a;++v)if(r=(s=e.features[v]).element){s=s.data;for(u=0;u<f;++u){i=(k=g[u]).value;if(typeof i==="function")i=i.call(null,s);i==null?k.name.local?r.removeAttributeNS(k.name.space,
k.name.local):r.removeAttribute(k.name):k.name.local?r.setAttributeNS(k.name.space,k.name.local,i):r.setAttribute(k.name,i)}for(u=0;u<j;++u){i=(k=c[u]).value;if(typeof i==="function")i=i.call(null,s);i==null?r.style.removeProperty(k.name):r.style.setProperty(k.name,i,k.priority)}if(i=m){if(typeof i==="function")i=i.call(null,s);for(;r.lastChild;)r.removeChild(r.lastChild);i!=null&&r.appendChild(t.svg("title")).appendChild(document.createTextNode(i))}}}var g=[],c=[],m;d.attr=function(e,a){g.push({name:ha(e),
value:a});return d};d.style=function(e,a,f){c.push({name:e,value:a,priority:arguments.length<3?null:f});return d};d.title=function(e){m=e;return d};return d}})(org.polymaps);
